...
import com.example.ecologic_route_ws.Models.Weather;
import com.example.ecologic_route_ws.OntologyService;
import com.example.ecologic_route_ws.SPARQLQueries;
...
public class WeatherConditionController {
  ...
  /*  @PostMapping("/createWeather")
    ...
    public ResponseEntity<String> deleteWeather(@PathVariable("id") String weatherId) {
        OntModel model = ontologyService.getModel();
        String namespace = "http://www.semanticweb.org/imenfrigui/ontologies/2024/8/PlanificateurTrajetsEcologiques#";
        String weatherURI = namespace + weatherId;

        Individual weatherIndividual = model.getIndividual(weatherURI);

        if (weatherIndividual != null) {
            weatherIndividual.remove();
            ontologyService.saveModel();
            return ResponseEntity.ok("Weather condition deleted successfully.");
        } else {
